...
args = {
    'dataset': 'OGB_MAG',
    'model_name': 'GIGHAE_lr0.001_dropout0.5_seed_0_patience20',
...
if __name__ == '__main__':
    ...
    model = nn.Sequential(gihgae, classifier)
    ...
    for epoch in range(args['epochs']):
        ...
        for batch, (input_nodes, output_nodes, blocks) in enumerate(train_loader_tqdm):
            ...
            # target node relation representation in the heterogeneous graph
            # 根据dataloader的采样策略不同，每个block代表不同阶的邻居
            input_features = {(stype, etype, dtype): blocks[0].srcnodes[dtype].data['feat'] for stype, etype, dtype in
                              blocks[0].canonical_etypes}

            nodes_representation, raw = model[0](blocks, copy.deepcopy(input_features))
            ...
            loss_reconstruction = F.mse_loss(nodes_representation[args['predict_category']], raw[args['predict_category']])
            loss_classification = F.cross_entropy(train_y_predict, train_y_true)
            loss = args['alpha'] * loss_reconstruction + loss_classification
        ...
        train_accuracy, train_micro_f1, train_macro_f1 = evaluate_node_classification(
        ...
        print(
            ...
            f'accuracy {train_accuracy:.4f}, micro f1 {train_micro_f1}, macro f1 {train_macro_f1:.4f}, \n'
            f'valid loss: {val_total_loss:.4f}, '
            f'accuracy {val_accuracy:.4f}, micro f1 {val_micro_f1}, macro f1 {val_macro_f1:.4f} \n'
            f'test loss: {test_total_loss:.4f}, '
            f'accuracy {test_accuracy:.4f}, micro f1 {test_micro_f1}, macro f1 {test_macro_f1:.4f}')
    ...
    print(f'final train accuracy: {train_accuracy:.4f}, micro f1 {train_micro_f1}, macro f1 {train_macro_f1:.4f}')
    ...
    print(f'final valid accuracy {val_accuracy:.4f}, micro f1 {val_micro_f1}, macro f1 {val_macro_f1:.4f}')
    ...
    test_accuracy, test_micro_f1, test_macro_f1 = evaluate_node_classification(predicts=test_y_predicts.argmax(dim=1),
                                                                               labels=test_y_trues)
    print(f'final test accuracy {test_accuracy:.4f}, micro f1 {test_micro_f1}, macro f1 {test_macro_f1:.4f}')
